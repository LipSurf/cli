...
const configPath = typescript_1.findConfigFile(process.cwd(), typescript_1.sys.fileExists, "tsconfig.json");
...
function compile(fileNames) {
    ...
    const configFile = typescript_1.readConfigFile(configPath, typescript_1.sys.readFile);
    const parsedConfig = typescript_1.parseJsonConfigFileContent(configFile.config, parseConfigHost, "./");
    // rootDir: src ensures that we consistently make the plugin directories in dist/tmp/
    let program = typescript_1.createProgram(fileNames, Object.assign(Object.assign({}, parsedConfig.options), { rootDir: "src" }));
...
function watch(fileNames, cb) {
    ...
    // a set of root files.
    const host = typescript_1.createWatchCompilerHost(configPath, {}, typescript_1.sys, createProgram, reportDiagnostic, reportWatchStatusChanged);
    ...
    typescript_1.createWatchProgram(host);
}
exports.watch = watch;
function reportDiagnostic(diagnostic) {
    console.error("Error", diagnostic.code, ":", typescript_1.flattenDiagnosticMessageText(diagnostic.messageText, formatHost.getNewLine()));
}
/**
...
function reportWatchStatusChanged(diagnostic) {
    console.info(typescript_1.formatDiagnostic(diagnostic, formatHost));
}
