...
var ParsedPlugin = /** @class */ (function () {
    ...
    ParsedPlugin.prototype.getExportName = function () {
        return this.ast.find(this.j.ExportDefaultDeclaration).get(0).node
            .declaration.name;
    };
    ParsedPlugin.prototype.getPluginDef = function () {
        ...
        startover: while (curVarDecl) {
            for (var _i = 0, propsToFind_1 = propsToFind; _i < propsToFind_1.length; _i++) {
                ...
                if (!curVarDecl.find(this.j.Property, { key: { name: prop } }).at(0)
                    .length) {
                    i++;
    ...
    ParsedPlugin.prototype.getCS = function (matching, buildForPlan) {
        ...
        if (commandsProps.size() === 0 &&
            !this.getTopLevelLiteral("init") &&
            !this.getTopLevelLiteral("destroy"))
            return "";
    ...
    ParsedPlugin.prototype.getVersion = function () {
        ...
        return this.pluginDef
            ...
            // hacky
            // dev build
            var typeA = x.parentPath.node ==
                _this.pluginDef.get(0).node.init.properties[1].argument;
            // prod build
            var typeB = x.get(0).parentPath.parentPath.parentPath.parentPath.node ==
                _this.pluginDef.get(0).node;
            return typeA || typeB;
        })
            .find(this.j.Literal)
            .get(0).node.value;
    ...
    ParsedPlugin.prototype.replaceCmdsAbovePlan = function (commandsObjs, pluginPlan, buildForPlan) {
        ...
        var replaced = commandsObjs
            ...
            var cmdPlan = (_a = planProp === null || planProp === void 0 ? void 0 : planProp.value) === null || _a === void 0 ? void 0 : _a.value;
            ...
            if (!pageFnProp) {
                cmdObj.node.properties.push(_this.j.property("init", _this.j.identifier("pageFn"), _this.j.template
                    .expression(templateObject_2 || (templateObject_2 = __makeTemplateObject(["()=>showNeedsUpgradeError({plan: ", "})"], ["()=>showNeedsUpgradeError({plan: ", "})"])), minNeededPlan.toString())));
            }
            return cmdObj;
        }, this.j.Property)
            ...
            // @ts-ignore
            .replaceWith(function (cmdObj) {
            var _a;
            return _this.j.template
                .expression(templateObject_3 || (templateObject_3 = __makeTemplateObject(["()=>showNeedsUpgradeError({plan: ", "})"], ["()=>showNeedsUpgradeError({plan: ", "})"])), (_a = cmdObj.parent.parent.data) === null || _a === void 0 ? void 0 : _a.minNeededPlan.toString());
        });
    ...
    ParsedPlugin.prototype.removeLanguageCode = function () {
        ...
        return this.ast
            .find(this.j.ExpressionStatement, {
            expression: {
                left: {
                    object: {
                        object: { name: this.exportName },
    ...
    ParsedPlugin.prototype.removeSimpleCommandProps = function (commandsProps) {
        return commandsProps
            .filter(function (x) {
            return COMMAND_PROPS_TO_REMOVE.includes(x.node.key.name);
        })
            .remove();
    };
    ...
    ParsedPlugin.prototype.getTopLevelProp = function (name) {
        ...
        return this.pluginDef
            ...
            .filter(function (x) {
            return x.parentPath.node ==
                _this.pluginDef.get(0).node.init.properties[1].argument;
        });
    };
    ParsedPlugin.prototype.getTopLevelLiteral = function (name) {
        var topLevelProp = this.getTopLevelProp(name).find(this.j.Literal);
    ...
    ParsedPlugin.prototype.getCommandsColl = function () {
        var cmds = this.getTopLevelProp("commands")
            .find(this.j.ArrayExpression)
            .at(0);
        return cmds;
    };
    ParsedPlugin.prototype.getCommandsObjs = function (commandsColl) {
        var cmdObjs = commandsColl
            .find(this.j.ObjectExpression)
    ...
    ParsedPlugin.prototype.getCommandsProps = function (commandsObjs) {
        var _this = this;
        var cmdProps = commandsObjs.map(function (cmdPath) {
            return _this.j(cmdPath)
                .find(_this.j.Property)
                .filter(function (p) {
                return lodash_1.get(p, "parentPath.parentPath.parentPath.parentPath.parentPath.value.key.name") === "commands";
            })
                .paths();
        }, this.j.Property);
        return cmdProps;
    ...
    ParsedPlugin.prototype.transformMatchStrs = function (commandsProps) {
        var _this = this;
        var matchProp = commandsProps.filter(function (x) {
            return x.node.type === "Property" && x.node.key.name == "match";
        });
        // remove matchStrs but not dynamic match fns
        matchProp
            .filter(function (x) {
            return x.node.value.type === "Literal" ||
                x.node.value.type === "ArrayExpression";
        })
            .remove();
        var dynMatchProp = matchProp.filter(function (x) { return x.node.value && x.node.value.type === "ObjectExpression"; });
        ...
        var langCmdsByLang = otherLangs.reduce(function (memo, lang) {
            var _a;
            return (__assign(__assign({}, memo), (_a = {},
                _a[lang] = _this.ast
                    .find(_this.j.AssignmentExpression, {
                    right: { type: "ObjectExpression" },
                    left: {
                        property: { name: lang },
                        type: "MemberExpression",
                        object: { property: { name: "languages" } },
                    },
                })
                    .find(_this.j.Property, { key: { name: "commands" } }),
                _a)));
        }, {});
        ...
        dynMatchProp.replaceWith(function (p) {
            ...
            var cmdName = p.parentPath.value.filter(function (x) { return x.key.name === "name"; })[0]
                .value.value;
            var addLangs = otherLangs
                .map(function (lang) {
                var matchFn = langCmdsByLang[lang].find(_this.j.Property, {
                    key: { value: cmdName },
                });
                // HACK
                if (!matchFn.length) {
                    matchFn = langCmdsByLang[lang].find(_this.j.Property, {
                        key: { name: cmdName },
                    });
                }
                var step2 = matchFn.find(_this.j.Property, { key: { name: "fn" } });
                if (step2.length) {
                    return _this.j.property("init", _this.j.identifier(lang), step2.get(0).node.value);
                }
            })
                .filter(function (x) { return x; });
            var matchObj = __spreadArrays([
                _this.j.property("init", _this.j.identifier("en"), p.value.value.properties[0].value)
            ], addLangs);
    ...
    ParsedPlugin.prototype.commandArrayToObject = function (commandsObjs, commandsColl) {
        ...
        var cmdsByName = commandsObjs
            .nodes()
            .filter(function (x) { return x; })
            .map(function (cmdPath) {
            // name for object literal without quotes
            // value for with quotes
            var nameProp = (cmdPath.properties.find(function (x) {
                return x.key.name === "name" ||
                    x.key.value === "name";
            }));
            var name = nameProp.value.value;
            var otherProps = cmdPath.properties.filter(function (x) {
                return x.key.name !== "name" &&
                    x.key.value !== "name";
            });
    ...
    ParsedPlugin.prototype.removeSimplePluginProps = function () {
        ...
        return (this.pluginDef
            ...
            .filter(function (x) {
            return PLUGIN_PROPS_TO_REMOVE.includes(x.node.key.name);
        })
            .remove());
    };
    return ParsedPlugin;
