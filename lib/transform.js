...
async function makePlugin(pluginId, pluginWLanguageFiles, resolveDir, prod = false, baseImports = true, define = {}) {
    ...
    try {
        ...
        // make backend
        const backend = `${resolvedPluginCode.substring(0, pluginSrcReplacementStartI)}${removeReplacedCallArtifacts(uneval(makeBackend(parsedPluginObj)))}`.replace(new RegExp(`var ${pluginId}_default\s*=`), "export default ");
