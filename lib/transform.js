...
exports.transformJSToPlugin = void 0;
/*;
...
const constants_cjs_1 = require("@lipsurf/common/constants.cjs");
...
const util_cjs_1 = require("@lipsurf/common/util.cjs");
...
const importPluginBase = `import PluginBase from '${constants_cjs_1.EXT_RESOURCES_PREFIX}/dist/modules/plugin-base.js';`;
const importExtensionUtil = `import ExtensionUtil from '${constants_cjs_1.EXT_RESOURCES_PREFIX}/dist/modules/extension-util.js';`;
...
function makeCS(plugin, plan, type) {
    ...
    // array to obj
    // @ts-ignore
    plugin.commands = (0, lodash_1.mapValues)((0, lodash_1.keyBy)(plugin.commands, "name"), (v) => (0, lodash_1.omit)(v, "name"));
...
function transformJSToPlugin(pluginId, globbedTs, outdir, prod, baseImports, define) {
    ...
    return makePlugin(pluginId, pluginWLanguageFiles, resolveDir, prod, baseImports, define)
        ...
        return Promise.all(res[0].map((c, i) => c
            ? fs_1.promises.writeFile(`${(0, path_1.join)(outdir, pluginId)}.${version}.${constants_cjs_1.PLANS[i]}.ls`, c, "utf8")
            : undefined));
    })
        .catch((e) => {
...
const ARTIFACT_REGX = new RegExp(`\\(${FN_ESCAPE_TAG}${(0, util_cjs_1.escapeRegex)(FN_ESCAPE_PREFIX)}(.*?)${(0, util_cjs_1.escapeRegex)(FN_ESCAPE_SUFFIX)}`, "gs");
...
async function makePlugin(pluginId, pluginWLanguageFiles, resolveDir, prod = false, baseImports = true, define = {}) {
    ...
    try {
        ...
        const byPlanAndMatching = {
            [constants_cjs_1.FREE_PLAN]: {},
            [constants_cjs_1.PLUS_PLAN]: {},
            [constants_cjs_1.PREMIUM_PLAN]: {},
        };
        ...
        let cloned = (0, clone_1.default)(parsedPluginObj, false);
        for (const plan of constants_cjs_1.PLANS) {
            let type;
            for (type of Object.values(util_1.PluginPartType).filter((x) => !isNaN(Number(x)))) {
              ...
              return ${pluginId}_default; })();`
                ...
                if (i != 5) {
                    i++;
                    cloned = (0, clone_1.default)(parsedPluginObj, false);
                }
                else {
        ...
        const transformedPluginsTuple = [
            resolvedPluginCode,
            ...constants_cjs_1.PLANS.reduce((memo, p) => memo.concat([
        ...
        for (let i = 0; i < constants_cjs_1.PLANS.length; i++) {
            const matchingNonMatching = splitPluginTuple.slice(i * 2 + 1, (1 + i) * 2 + 1);
            if (constants_cjs_1.PLANS[i] !== constants_cjs_1.FREE_PLAN &&
                // @ts-ignore
                matchingNonMatching.reduce((memo, x) => memo + x.length, 0) === 0)
                // no plugin for this level
                finalPluginsTuple.push("");
            else
                finalPluginsTuple.push([baseImportsStr + splitPluginTuple[0], ...matchingNonMatching].join(constants_cjs_1.PLUGIN_SPLIT_SEQ));
        }
