...
const importPluginBase = `import PluginBase from 'chrome-extension://${constants_cjs_1.EXT_ID}/dist/modules/plugin-base.js';`;
const importExtensionUtil = `import ExtensionUtil from 'chrome-extension://${constants_cjs_1.EXT_ID}/dist/modules/extension-util.js';`;
...
function transformJSToPlugin(pluginId, globbedTs, outdir, baseImports, define) {
    ...
    return makePlugin(pluginId, pluginWLanguageFiles, resolveDir, process.env.NODE_ENV === "production", baseImports, define)
        .then((res) => {
        const version = versionConvertDots(res[1]);
        return Promise.all(res[0].map((c, i) => c
            ? fs_1.promises.writeFile(`${path_1.join(outdir, pluginId)}.${version}.${constants_cjs_1.PLANS[i]}.ls`, c, "utf8")
            : undefined));
    })
        .catch((e) => {
...
const ARTIFACT_REGX = new RegExp(`\\(${FN_ESCAPE_TAG}${util_cjs_1.escapeRegx(FN_ESCAPE_PREFIX)}(.*?)${util_cjs_1.escapeRegx(FN_ESCAPE_SUFFIX)}`, "gs");
