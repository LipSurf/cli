...
async function build(options, plugins = "") {
    ...
    try {
        envVars = util_1.getDotEnv(path_1.default.join(envFile));
    }
    catch (e) {
        console.warn(`No "${envFile}" file found.`);
    }
    const define = lodash_1.transform(Object.assign({ NODE_ENV: IS_PROD ? "production" : "development" }, envVars), (r, val, key) => (r[`process.env.${key}`] = `"${util_1.escapeQuotes(val)}"`));
    if (options.watch) {
        if (options.check) {
