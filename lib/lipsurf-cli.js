...
const dev_cjs_1 = require("lipsurf-common/dev.cjs");
...
function forkAndTransform(pluginIds, ...args) {
    return new Promise((cb) => {
        if (pluginIds.length === 1) {
            // @ts-ignore
            transform_1.transformJSToPlugin(pluginIds[0], ...args).finally(cb);
        }
        else {
            let finishedForks = 0;
            const forks = [];
            for (let pluginId of pluginIds) {
                ...
                const forked = child_process_1.fork(path_1.default.join(__dirname, "./worker.js"));
                forks.push(forked);
...
async function build(options, plugins = "") {
    ...
    try {
        envVars = dev_cjs_1.getDotEnv(path_1.default.join(envFile));
    }
    catch (e) {
        console.warn(`No "${envFile}" file found.`);
    }
    const define = lodash_1.transform(Object.assign({ NODE_ENV: IS_PROD ? "production" : "development" }, envVars), (r, val, key) => (r[`process.env.${key}`] = `"${dev_cjs_1.escapeQuotes(val)}"`));
    if (options.watch) {
        if (options.check) {
            ts_compile_1.watch(globbedTs, async () => {
                console.log("Starting transform...");
                await forkAndTransform(pluginIds, globbedTs, options.outDir, options.baseImports, define);
        ...
        if (options.check)
            await ts_compile_1.compile(globbedTs);
        else {
            await transpileFiles(globbedTs);
        }
        await forkAndTransform(pluginIds, globbedTs, options.outDir, options.baseImports, define);
...
function transpileFiles(globbedTs) {
    return Promise.all(globbedTs.map((f) => core_1.transformFile(f, {
        jsc: {
            parser: {
                syntax: "typescript",
                dynamicImport: true,
...
async function upVersion(options) {
    ...
    try {
        child_process_2.execSync("git diff-index --ignore-submodules --quiet HEAD -- ./").toString();
    }
    catch (e) {
    ...
    let files;
    try {
        files = await fs_extra_1.default.readdir(parDir);
    }
    catch (e) {
    ...
    const oldVersion = (await evaluator_1.evalPlugin(await fs_extra_1.default.readFile(`./${TMP_DIR}/${anyPluginName}/${anyPluginName}.js`, "utf8"), `./${TMP_DIR}/${anyPluginName}/`)).version;
    ...
    child_process_2.execSync(`sed -i 's/version: "${oldVersion}"/version: "${newVersion}"/g' src/*/*.ts`);
    await build(options);
    // remove the old plugins
    try {
        child_process_2.execSync(`rm dist/*.${oldVersion.replace(/\./g, "-")}.*.ls`);
    }
    catch (e) {
    ...
    child_process_2.execSync("git add src dist");
