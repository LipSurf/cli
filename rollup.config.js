...
module.exports = function getConfig(finalOutputDir) {
	return [].concat(...globby.sync(['src/*/*.ts', '!src/*/tests.ts', '!src/@types', '!src/*/*.*.ts']).map(fileName => {
		...
		if (folderName) {
			return [
				...
				...PLANS_AND_PARTS.map(planAndPart => `dist/tmp/${folderName}.2-5-0.${planAndPart}.js`).map(filename => ({
				...
				})),
				{
					...
					input: PLANS_AND_PARTS.map(planAndPart => `dist/tmp/${folderName}.2-5-0.${planAndPart}.resolved.js`),
					plugins: [
						makeCS(),
					],
					output: {
