...
module.exports = function getConfig(finalOutputDir, pluginNames) {
	return [].concat(...globby.sync(['src/*/*.ts', '!src/*/tests.ts', '!src/@types', '!src/*/*.*.ts']).map(filePath => {
		let regxRes = FOLDER_REGX.exec(filePath);
		if (regxRes) {
			let [_, folderName, fileName] = regxRes;
			// last check is for unincluding eg utils.ts
			if (folderName && (!pluginNames.length || pluginNames.includes(folderName)) && fileName.startsWith(folderName)) {
				console.log('doing ' + folderName);
				return [
					{
						input: [
							`src/${folderName}/${folderName}.ts`,
							`src/${folderName}/${folderName}.*.ts`,
						],
						treeshake: false,
						plugins: [
					...
					...PLANS_AND_PARTS.map(planAndPart => `dist/tmp/${folderName}.2-6-0.${planAndPart}.js`).map(filename => ({
					...
					})),
					{
						...
						input: PLANS_AND_PARTS.map(planAndPart => `dist/tmp/${folderName}.2-6-0.${planAndPart}.resolved.js`),
						plugins: [
							makeCS(),
						],
						output: {
