...
//         if (
//           !curVarDecl.find(this.j.Property, { key: { name: prop } }).at(0)
//             .length
//         ) {
//           i++;
...
//         this.j.property(
//           "init",
//           this.j.identifier("languages"),
//           this.j.template.expression`{}`
//         )
//       );
...
//       commandsObjs
//         .filter((cmdObj) => {
//           const globalProp = (<Property[]>cmdObj.value.properties).find(
//             (prop: Property) => (<Identifier>prop.key).name === "global"
//           );
...
//     if (
//       this.replaceCmdsAbovePlan(commandsObjs, this.pluginPlan, buildForPlan)
//     ) {
...
//         const typeA =
//           x.parentPath.node ==
//           this.pluginDef.get(0).node.init.properties[1].argument;
//         // prod build
//         const typeB =
//           x.get(0).parentPath.parentPath.parentPath.parentPath.node ==
//           this.pluginDef.get(0).node;
//         return typeA || typeB;
//       })
//       .find(this.j.Literal)
//       .get(0).node.value;
...
//    */
//   private replaceCmdsAbovePlan(
//     commandsObjs: Collection<ObjectExpression>,
//     pluginPlan: number,
//     buildForPlan: number
//   ): boolean {
...
//       .map((cmdObj) => {
//         const planProp = (<Property[]>cmdObj.value.properties).find(
//           (prop: Property) => (<Identifier>prop.key).name === "plan"
//         );
...
//           else if (pluginPlan > buildForPlan) return true;
//         } else {
//           if (buildForPlan === cmdPlan) cmdsOnThisPlan = true;
//           if (minNeededPlan > buildForPlan) return true;
//         }
//         return false;
//       })
//       .map((cmdObj) => {
...
//         const pageFnProp = (<Property[]>cmdObj.value.properties).find(
//           (prop: Property) => (<Identifier>prop.key).name === "pageFn"
//         );
//         if (!pageFnProp) {
//           cmdObj.node.properties.push(
//             this.j.property(
//               "init",
//               this.j.identifier("pageFn"),
//               this.j.template
//                 .expression`()=>showNeedsUpgradeError({plan: ${minNeededPlan.toString()}})`
//             )
//           );
//         }
//         return cmdObj;
...
//         (cmdObj) =>
//           this.j.template
//             .expression`()=>showNeedsUpgradeError({plan: ${cmdObj.parent.parent.data?.minNeededPlan.toString()}})`
//       );
...
//       .filter((x) =>
//         COMMAND_PROPS_TO_REMOVE.includes((<Identifier>x.node.key).name)
//       )
//       .remove();
//   }
...
//     const topLevelProp = this.getTopLevelProp(name).find(this.j.Literal);
...
//   private getCommandsObjs(
//     commandsColl: Collection<ArrayExpression>
//   ): Collection<ObjectExpression> {
//     const cmdObjs = commandsColl
//       .find(this.j.ObjectExpression)
//       // restrict to the correct depth
//       .filter(
//         (x) => x.parentPath.parentPath === commandsColl.get(0).parentPath
//       );
//     return cmdObjs;
//   }

//   private getCommandsProps(
//     commandsObjs: Collection<ObjectExpression>
//   ): Collection<Property> {
//     const cmdProps = commandsObjs.map(
//       (cmdPath) =>
//         this.j(cmdPath)
//           .find(this.j.Property)
//           .filter(
...
//               get(
//                 p,
//                 "parentPath.parentPath.parentPath.parentPath.parentPath.value.key.name"
//               ) === "commands"
//           )
//           .paths(),
//       this.j.Property
...
//     const matchProp = commandsProps.filter(
//       (x) =>
//         x.node.type === "Property" && (<Identifier>x.node.key).name == "match"
//     );
//     // remove matchStrs but not dynamic match fns
//     matchProp
//       .filter(
//         (x) =>
//           x.node.value.type === "Literal" ||
//           x.node.value.type === "ArrayExpression"
//       )
//       .remove();

//     const dynMatchProp = matchProp.filter(
//       (x) => x.node.value && x.node.value.type === "ObjectExpression"
//     );
...
//     const langCmdsByLang = otherLangs.reduce(
//       (memo, lang) => ({
//         ...memo,
//         ...{
//           [lang]: this.ast
//             .find(this.j.AssignmentExpression, {
...
//       const cmdName = p.parentPath.value.filter((x) => x.key.name === "name")[0]
//         .value.value;
...
//         this.j.property(
//           "init",
//           this.j.identifier("en"),
//           (<Property>(<ObjectExpression>p.value.value).properties[0]).value
//         ),
//         ...addLangs,
//       ];
//       return this.j.property(
//         "init",
//         this.j.identifier("match"),
//         this.j.objectExpression(matchObj)
//       );
//     });
//   }

//   // cmds array to object with cmd names as keys
//   private commandArrayToObject(
//     commandsObjs: Collection<ObjectExpression>,
//     commandsColl: Collection<ArrayExpression>
//   ) {
//     // filter out already removed nodes
//     const cmdsByName = commandsObjs
//       .nodes()
//       .filter((x) => x)
//       .map((cmdPath) => {
//         // name for object literal without quotes
//         // value for with quotes
//         let nameProp = <ObjectProperty>(
//           cmdPath.properties.find(
//             (x) =>
//               (<Identifier>(<ObjectProperty>x).key).name === "name" ||
//               (<Literal>(<ObjectProperty>x).key).value === "name"
//           )
//         );
//         let name = <string>(<Literal>nameProp.value).value;
//         let otherProps = cmdPath.properties.filter(
//           (x) =>
//             (<Identifier>(<ObjectProperty>x).key).name !== "name" &&
//             (<Literal>(<ObjectProperty>x).key).value !== "name"
//         );
...
//           "init",
//           this.j.literal(name),
//           this.j.objectExpression(otherProps)
//         );
//       });
...
//         })
//         .filter((x) =>
//           PLUGIN_PROPS_TO_REMOVE.includes((<Identifier>x.node.key).name)
//         )
//         .remove()
//     );
//   }
